...
#define iPrepass                0      
#define iWhitehole              0      
#define iInWhichUniverse        0      
#define iGrid                   0      
#define iEnableHeatHaze         1      
#define iEnableShadowCulling    0      
#define iObserverMode           0      
#define iPolarization           0      
#define iQuality                1.0    
#define CONST_M (0.5 * iBhSize)

#define iBlackHoleTime          (2.0 * iTime * g_diskRotSpeed)
...
#define iPolarizationAngle      0.0       
#define iHeatHaze               1.0       
...
// Star Nest by Pablo Roman Andrioli (Adapted for 3D)
...
vec4 SampleBackground(vec3 Dir, float Shift, float Status) {
    if (g_showBackground == 0) return vec4(0.0);

    float time = iTime * speed * (g_stopBackground > 0.5 ? 0.0 : 1.0) + .25;
    vec3 from = vec3(1., .5, 0.5);
    from += vec3(time * 2., time, -2.);
    
    // Instead of using UV and mouse to construct a ray, we use the geodesic Ray Dir directly
    vec3 dir = Dir;
    ...
    vec4 Backcolor = vec4(v * .01, 1.);
    
    // Apply Doppler shift colors as in original code
...
vec3 GetHazeForce(vec3 pos_Rg, float time, float PhysicalSpinA, float PhysicalQ, float InterRadius, float OuterRadius, float Thin, float Hopper, float AccretionRate) {
    ...
    float jetHazeStrength = (AccretionRate >= 1e-6) ? clamp((log(AccretionRate) - log(1e-6)) / (log(1.0) - log(1e-6)), 0.0, 1.0) : 0.0;

    if (diskHazeStrength <= 0.001 && jetHazeStrength <= 0.001) return vec3(0.0);

    vec3 totalForce = vec3(0.0); float eps = 0.1;
    float rotSpeedBase = 100.0 * HAZE_ROT_SPEED * g_diskRotSpeed; float jetSpeedBase = 50.0 * HAZE_FLOW_SPEED;
    float ReferenceOmega = GetKeplerianAngularVelocity(6.0 * iBhSize, iBhSize, PhysicalSpinA, PhysicalQ);
    ...
    if (diskHazeStrength > 0.001) {
        ...
        if (maskDisk > 0.001) {
            float omega = GetKeplerianAngularVelocity(length(pos_Rg.xz), iBhSize, PhysicalSpinA, PhysicalQ);
        ...
        if (shouldSample) {
            ...
            if (abs(PosY) < max(GeometricThin * 1.5, max(0.0, InnerCloudBound)) && PosR < OuterRadius && PosR > InterRadius) {
                 ...
                 if ((abs(PosY) < PerturbedThickness) || (abs(PosY) < InnerCloudBound)) {
                     ...
                     SampleColor.xyz *= BrightWithoutRedshift * KelvinToRgb(VisionTemperature) * min(pow(FreqRatio, RedShiftIntensityExponent), ShiftMax) * min(1.0, 1.3 * (OuterRadius - PosR) / (OuterRadius - InterRadius)); SampleColor.xyz *= g_iDiskColor; SampleColor.a *= 0.125;
...
vec4 JetColor(vec4 BaseColor, vec4 RayPos, vec4 LastRayPos, vec4 iP_cov, vec4 lastiP_cov, float iE_obs, float InterRadius, float OuterRadius, float JetRedShiftIntensityExponent, float JetBrightmut, float JetReddening, float JetSaturation, float AccretionRate, float JetShiftMax, float PhysicalSpinA, float PhysicalQ, bool isoutgoing, inout float RayMarchPhase) 
{
    ...
    while (TraveledDist < TotalDist && SafetyLoopCount < MaxLoops) {
        ...
        if (shouldSample) {
            ...
            if (RhoSq < 2.0 * InterRadius * InterRadius + pow(0.03 * iJetWidth, 2.0) * PosY * PosY && PosR < sqrt(2.0) * OuterRadius * iJetLength) {
                InJet = true; float ShapeVal = 1.0 / sqrt(max(1e-9, InterRadius * InterRadius + pow(0.02 * iJetWidth, 2.0) * PosY * PosY));
                AccumColor += vec4(1.0, 1.0, 1.0, 0.5) * max(0.0, 1.0 - 5.0 * ShapeVal * abs(1.0 - pow(Rho * ShapeVal, 2.0))) * ShapeVal * mix(0.7 + 0.3 * PerlinNoise1D(0.3 * (EmissionTime - 1.0 / 0.8 * abs(abs(PosY) + 100.0 * (RhoSq / max(0.1, PosR)))) / max(1e-6, (OuterRadius / 100.0)) / (1.0 / 0.8)), 1.0, exp(-0.01 * 0.01 * PosY * PosY)) * max(0.0, 1.0 - 1.0 * exp(-0.0001 * PosY / max(1e-6, InterRadius) * PosY / max(1e-6, InterRadius))) * exp(-4.0 / (2.0) * PosR / max(1e-6, OuterRadius * iJetLength) * PosR / max(1e-6, OuterRadius * iJetLength)) * 0.5 * StepSize; 
            }

            float Wid = abs(PosY);
            if (Rho < (1.3 * InterRadius + 0.25 * Wid) * iJetWidth && Rho > (0.7 * InterRadius + 0.15 * Wid) * iJetWidth && PosR < 30.0 * InterRadius * iJetLength) {
                InJet = true; float ShapeVal = 1.0 / max(1e-9, (InterRadius + 0.2 * Wid * iJetWidth));
                vec2 TwistedPos = SamplePos.xz + 0.2 * iJetWidth * (1.1 - exp(-0.1 * 0.1 * PosY * PosY)) * (PerlinNoise1D(0.35 * (EmissionTime - 1.0 / 0.8 * abs(PosY)) / (1.0 / 0.8)) - 0.5) * vec2(cos(0.666666 * 2.0 * GetKeplerianAngularVelocity(InterRadius, 1.0, PhysicalSpinA, PhysicalQ) * (EmissionTime - 1.0 / 0.8 * abs(PosY))), -sin(0.666666 * 2.0 * GetKeplerianAngularVelocity(InterRadius, 1.0, PhysicalSpinA, PhysicalQ) * (EmissionTime - 1.0 / 0.8 * abs(PosY))));
                AccumColor += vec4(1.0, 1.0, 1.0, 0.5) * max(0.0, 1.0 - 2.0 * abs(1.0 - pow(length(TwistedPos) * ShapeVal, 2.0))) * ShapeVal * (1.0 - exp(-PosY / max(1e-6, InterRadius) * PosY / max(1e-6, InterRadius))) * exp(-0.005 * PosY / max(1e-6, InterRadius * iJetLength) * PosY / max(1e-6, InterRadius * iJetLength)) * 0.5 * StepSize; 
            }
            ...
            if (InJet) {
                ...
                AccumColor.xyz *= KelvinToRgb(min(100000.0 * FreqRatio,100000.0)) * min(pow(FreqRatio, JetRedShiftIntensityExponent), JetShiftMax) * JetBrightmut; AccumColor.xyz *= g_iDiskColor; AccumColor.a *= 0.0; 
...
vec4 GridColorSimple(vec4 BaseColor, vec4 RayPos, vec4 LastRayPos, float PhysicalSpinA, float PhysicalQ, bool isoutgoing, float EndStepSign) {
    ...
    float HorizonDiscrim = CONST_M * CONST_M - PhysicalSpinA * PhysicalSpinA - PhysicalQ * PhysicalQ;
    if (CheckPositive) {
        SignedGridRadii[GridCount] = 70.0; GridColors[GridCount++] = 0.3*vec3(0.0, 1.0, 1.0); 
        if (HorizonDiscrim >= 0.0) { SignedGridRadii[GridCount] = (CONST_M + sqrt(HorizonDiscrim)) * 1.06; GridColors[GridCount++] = 0.3*vec3(0.0, 1.0, 0.0); SignedGridRadii[GridCount] = (CONST_M - sqrt(HorizonDiscrim)) * 0.94; GridColors[GridCount++] = 0.3*vec3(1.0, 0.0, 0.0); }
    }
...
vec4 GridColor(vec4 BaseColor, vec4 RayPos, vec4 LastRayPos, vec4 iP_cov, float iE_obs, float PhysicalSpinA, float PhysicalQ, bool isoutgoing, float EndStepSign) {
    ...
    float HorizonDiscrim = CONST_M * CONST_M - PhysicalSpinA * PhysicalSpinA - PhysicalQ * PhysicalQ;
    if (StartStepSign > 0.0 || EndStepSign > 0.0) { SignedGridRadii[GridCount++] = (CONST_M + sqrt(max(0.0, HorizonDiscrim))) * 1.06; SignedGridRadii[GridCount++] = 20.0; if (HorizonDiscrim >= 0.0) { SignedGridRadii[GridCount++] = (CONST_M - sqrt(HorizonDiscrim)) * 0.94; } }
...
TraceResult TraceRay(vec2 FragUv, vec2 Resolution, mat4 iInverseCamRot, vec4 iBlackHoleRelativePosRs, vec4 iBlackHoleRelativeDiskNormal, vec4 iBlackHoleRelativeDiskTangen, float iUniverseSign)
{
    ...
    float HorizonDiscrim = CONST_M * CONST_M - PhysicalSpinA * PhysicalSpinA - PhysicalQ * PhysicalQ;
    float EventHorizonR = CONST_M + sqrt(max(0.0, HorizonDiscrim));
    float InnerHorizonR = CONST_M - sqrt(max(0.0, HorizonDiscrim));
    ...
    while (bShouldContinueMarchRay) {
        ...
        float dLambda = max(0.5 * (bIsNakedSingularity ? 1.0 : mix(max(abs(iSpin),0.1), 1.0, clamp((geo.r - InnerHorizonR) / (CONST_M - InnerHorizonR), 0.0, 1.0))) * (1.0 / (1.0 + 1.0 * ((PhysicalQ * PhysicalQ) / (geo.r2 + 0.01)))) * min(DistRing / (length(k1.X) + 1e-9), length(P_cov) / (length(k1.P) + 1e-15)), 1e-7); 
...
vec4 RenderTopologyMap(vec2 uv, vec3 camPos, vec3 camDir) {
    vec2 canvas_sz = vec2(300.0, 150.0);
    vec2 canvas_rel_sz = canvas_sz / iResolution.xy;
    vec2 canvas_rel_p0 = vec2(0.98) - canvas_rel_sz;
    ...
    vec2 centerSide = canvas_p0 + vec2(0.0, canvas_sz.y * 0.5); 
    ...
    if (abs(fragCoord.x - (canvas_p0.x + halfWidth)) < 1.5) color = mix(color, vec4(0.8, 0.8, 0.8, 1.0), 1.0); 
    ...
    if (fragCoord.x > centerSide.x && fragCoord.x < canvas_p0.x + halfWidth) {
        ...
        float dGdr = 2.0*r_px*(r2+a2*cosT2) + (r2+a2)*2.0*r_px + a2*sinT2*2.0*M; 
...
void mainImage( out vec4 FragColor, in vec2 FragCoord )
{
    ...
    int iBufWidth = int(iResolution.x);
    ...
    vec4 mapCol = vec4(0.0);
    if (g_showMap > 0) {
        mapCol = RenderTopologyMap(screenUV, vec3(CamPosWorld.x,-CamPosWorld.y,-CamPosWorld.z), vec3(mapCamDir.x,-mapCamDir.y,-mapCamDir.z));
    }
